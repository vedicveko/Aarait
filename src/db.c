...
void discrete_load(FILE * fl, int mode, char *filename)
{
  ...
  const char *modes[] = {"world", "mob", "obj", "qst"};
...
void renum_zone_table(void)
{
  int cmd_no, a, b, c, d, olda, oldb, oldc, oldd;
  zone_rnum zone;
  char buf[128];
...
void parse_simple_mob(FILE *mob_f, int i, int nr)
{
  ...
  mob_proto[i].points.max_move = 15;
  ...
  convert_copper(mob_proto + i, t[0]);
  ...
  mob_proto[i].mob_specials.newitem = 0;
...
void interpret_espec(const char *keyword, const char *value, int i, int nr)
{
  ...
  CASE("Size") {
    mob_proto[i].mob_specials.size = num_arg;
  }

  CASE("Weight") {
    RANGE(1, 1000);
    mob_proto[i].mob_specials.body_weight = num_arg;
// number(num_arg, (num_arg + mob_proto[i].player.level));
  }
...
char *parse_object(FILE * obj_f, int nr)
{
 ...
 if (t[4] < 0 || t[4] > 100) {
  obj_proto[i].obj_flags.curr_slots = 1;
  obj_proto[i].obj_flags.total_slots = 1;
 }
  else {
...
void load_zones(FILE * fl, char *zonename)
{
  ...
  for (;;) {
    ...
    error = 0;
    if (strchr("MOGEPD", ZCMD.command) == NULL) {	/* a 3-arg command */
...
void make_maze(int zone)
{
  ...
  sprintf(buf, "Building maze for zone #%d", zone);
  log(buf);
...
void reset_zone(zone_rnum zone)
{
  ...
  for (cmd_no = 0; ZCMD.command != 'S'; cmd_no++) {
    ...
    case 'M':			/* read a mobile */
      if ((mob_index[ZCMD.arg1].number < ZCMD.arg2) && ((ZCMD.arg4 > number(1, 100)) || (ZCMD.arg4 == 100))) {
	mob = read_mobile(ZCMD.arg1, REAL);
        if(GET_MOB_WEIGHT(mob) && !IS_NPC_FISH(mob) && !IS_NPC_PIG(mob)) {
          GET_MOB_WEIGHT(mob) = number(GET_MOB_WEIGHT(mob), (GET_MOB_WEIGHT(mob) + GET_LEVEL(mob)));
        }
        
        if(IS_NPC_FISH(mob)) {
           if(number(1, 20) == 1) {
              GET_MOB_WEIGHT(mob) = number(0, GET_MOB_WEIGHT(mob) * 2);
              if(!number(0, 5000)) {
                 GET_MOB_WEIGHT(mob) += number(0, 100);
              }
           }
           else {
              GET_MOB_WEIGHT(mob) = number(0, GET_MOB_WEIGHT(mob));
    ...
    case 'O':			/* read an object */
      if ((obj_index[ZCMD.arg1].number < ZCMD.arg2) && ((ZCMD.arg4 > number(1, 100)) || (ZCMD.arg4 == 100))) {
	if (ZCMD.arg3 >= 0) {
	...
	break;
      ...
      if ((obj_index[ZCMD.arg1].number < ZCMD.arg2) && ((ZCMD.arg4 > number(1, 100)) || (ZCMD.arg4 == 100))) {
	obj = read_object(ZCMD.arg1, REAL);
        if(IS_OBJ_STAT(obj, ITEM_BODY_PART)) {
           arg = number(1, (GET_MOB_WEIGHT(mob) * 10));
           GET_OBJ_WEIGHT(obj) = GET_MOB_WEIGHT(mob);
           GET_OBJ_COST(obj) = arg;
           GET_OBJ_RENT(obj) = arg;
...
void store_to_char(struct char_file_u * st, struct char_data * ch)
{
  ...
  ch->current_quest = st->current_quest;
  ch->num_completed_quests = st->num_completed_quests;
  ch->completed_quests = st->completed_quests;
...
void char_to_store(struct char_data * ch, struct char_file_u * st)
{
  ...
  st->num_completed_quests = ch->num_completed_quests;
  st->completed_quests = ch->completed_quests;
