...
ACMD(do_say)
{
	...
	break;
    case 3:
	sprintf(buf + strlen(buf), "sticks %s tounge out and ", HSHR(ch));
	sprintf(buf2 + strlen(buf2), "stick your tounge out and ");
	break;
    default:
	break;
    }
...
void perform_tell(struct char_data *ch, struct char_data *vict, char *arg)
{
	...
	act("$n says something to $N.", FALSE, ch, 0, vict, TO_NOTVICT);
    }
...
ACMD(do_tell)
{
	...
	send_to_char("Who do you wish to tell what??\r\n", ch);
	     ...
	     && !(vict =
		  get_player_vis(ch, buf,
				 FIND_CHAR_WORLD))) send_to_char(NOPERSON,
								 ch);
    else if (GET_LEVEL(ch) >= LVL_IMMORT
	     && !(vict =
		  get_char_vis(ch, buf,
...
ACMD(do_gen_comm)
{
	...
	if (COLOR_LEV(ch))
	    sprintf(buf1, "%sYou %s, '%s'%s", color_on,
...
ACMD(do_htell)
{
    ...
    if (!*argument) {
	send_to_char("What do you want to tell your house?", ch);
	return;
    }
	...
	sprintf(buf1, "/cWYou /cCtell your house, '%s'\r\n/c0", argument);
    send_to_char(buf1, ch);

    for (i = descriptor_list; i; i = i->next) {
	if (!i->connected && i != ch->desc) {
	    if (i->character->player_specials->saved.noble_house) {
		if (i->character->player_specials->saved.noble_house ==
		    GET_HOUSE(ch)) {

		    if (strcmp(i->character->player.name, ch->player.name)) {
			sprintf(buf, "/cC%s tells your house, '%s'\r\n/c0",
				(((IS_AFFECTED(ch, AFF_INVISIBLE)) &&
				  (!IS_AFFECTED(i->character,
						AFF_DETECT_INVIS))) ?
