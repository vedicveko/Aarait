...
char *print_rp_hit(struct char_data *ch);
char *print_rp_max_hit(struct char_data *ch);

extern const int fish_vnums[];
extern const char *fish_names[];
extern const char *rp_hit[];
extern const char *rp_max_hit[];
			...
			int mode)
{
  ...
  *buf = '\0';
  ...
  if ((mode == 0) && object->description)
    ...
    if(GET_OBJ_TYPE(object) == ITEM_FISH) {
        send_to_char("You spy a slimy fresh caught fish.\r\nBest you sell it before it goes bad!\r\n", ch);
        
        if(GET_OBJ_WEIGHT(object)) { 
         sprintf(buf, "It weighs about %d pounds.\r\n", GET_OBJ_WEIGHT(object));
        }
        else {
         sprintf(buf, "It weighs less than one pound.\r\n");
        }
        send_to_char(buf, ch);
  ...
  if (mode != 3) {
    if((GET_OBJ_TYPE(object) == ITEM_POLE) && (real_object(GET_OBJ_VAL(object, 0)))) { 
      obj = read_object(GET_OBJ_VAL(object, 0), VIRTUAL);
      sprintf(buf1, " baited with %s", obj->short_description);
      strcat(buf, buf1);
    } 
...
void look_at_char(struct char_data * i, struct char_data * ch)
{
  ...
  if (!ch->desc)
   ...
   if (i->player.description) {
    ...
    if(IS_NPC(i)) {
      if(GET_MOB_WEIGHT(i)) {
        sprintf(buf, "%s weighs about %d pounds.\r\n", CAP(GET_NAME(i)), GET_MOB_WEIGHT(i));
        send_to_char(buf, ch);
      }
      else {
        sprintf(buf, "%s weighs less than one pound.\r\n", CAP(GET_NAME(i)));
        send_to_char(buf, ch);
     }
   }
...
void list_one_char(struct char_data * i, struct char_data * ch)
{
  ...
  if (IS_NPC(i)) {
    ...
    if(!GET_RANK(i)) {
      sprintf(buf, "%s %s", i->player.name, no_house_noble_houses[(int)GET_HOUSE(i)]);
    }
    else {
      sprintf(buf, "%s %s %s", rank[(int)GET_RANK(i)][(int)GET_SEX(i)], i->player.name, no_house_noble_houses[(int)GET_HOUSE(i)]);
    }
  ...
  if(SECT(i->in_room) == SECT_WATER_NOSWIM) {
   for (obj = i->carrying; obj; obj = obj->next_content) {
    if (GET_OBJ_TYPE(obj) == ITEM_BOAT) {
       sprintf(buf1, " in %s.", obj->short_description);
       strcat(buf, buf1);
       found = 1;
       break;
    }
...
void look_at_room(struct char_data * ch, int ignore_brief)
{
  ...
  if ((!IS_NPC(ch) && !PRF_FLAGGED(ch, PRF_BRIEF)) || ignore_brief ||
      ...
      if(!IS_NPC(ch) && SECT(ch->in_room) == SECT_ACORN) {
send_to_char("Glints of sunlight cascade through the canopy of acorn tree\r\n", ch);
send_to_char("leaves above you. The acorn trees themselves have a gritty\r\n", ch);
send_to_char("brown bark and large three part leaves.  Fallen leaves from\r\n", ch);
send_to_char("the acorn trees make crunching noises as you walk.\r\n", ch);
      } 
      else if(ROOM_FLAGGED(ch->in_room, ROOM_VIEW_DESC)) {
        send_to_char(world[ch->in_room].description, ch);
      }
  }

  if(IS_SET(world[ch->in_room].room_flags, ROOM_FERRY)) {
      send_to_char("A ferry is tied to the dock.\r\n", ch);
  }

  if((RM_CHUM(ch->in_room)) && (SECT(ch->in_room) == SECT_WATER_NOSWIM || SECT(ch->in_room) == SECT_WATER_SWIM)) {
      send_to_char("The water has been chummed.\r\n", ch);
  }

  if(!IS_NPC(ch) && !IS_SET(world[ch->in_room].room_flags, ROOM_NOVIEW)) { 
      make_big_map(ch, "10");
  }
...
if(!(GET_OBJ_TYPE(tmp_object) == ITEM_CONTAINER && GET_OBJ_VAL(tmp_object, 3) == 1)) {
 // condition reports as a percentage now, so items can have different
 // starting tslots and cslots, eg. for wood and steel

 if(GET_OBJ_TYPE(tmp_object) != ITEM_POLE && GET_OBJ_TYPE(tmp_object) != ITEM_ARMOR && GET_OBJ_TYPE(tmp_object) != ITEM_WORN && GET_OBJ_TYPE(tmp_object) != ITEM_WEAPON) { return; }
 ...
 if (condition <= 90) {
   sprintf(buf, "This looks in extremely superior condition.\r\n");
   send_to_char(buf, ch);
   return;
   }
 else {
...
ACMD(do_gold)
{
 int amount = 0;
  ...
  if ((amount = (GET_PLATINUM(ch) + GET_GOLD(ch) + GET_SILVER(ch) + GET_COPPER(ch))) == 0)
    send_to_char("You're broke!\r\n", ch);
  else if (amount == 1)
    send_to_char("You have one miserable coin.\r\n", ch);
  else {
    sprintf(buf, "You have %d %s, %d %s, %d %s, and %d %s.\r\n", GET_PLATINUM(ch), coin_types[PLAT_COINS], GET_GOLD(ch), coin_types[GOLD_COINS], GET_SILVER(ch), coin_types[SILVER_COINS], GET_COPPER(ch), coin_types[COPPER_COINS]);
    send_to_char(buf, ch);
  }
}
...
ACMD(do_score)
{
  int fish = 0, i = 0, caught = 0;
  if (IS_NPC(ch))
    return;

sprintf(buf2, "%s %s", GET_NAME(ch), no_house_noble_houses[(int)GET_HOUSE(ch)]);

  sprintf(buf, "\r\n     /cYNAME:/cW %-20s            /cYAGE:/c0 %-3d/c0\r\n", buf2, GET_AGE(ch));
    sprintf(buf, "%s      /cYJOB:/cW %-17s              /cYRANK:/c0 %-5s\r\n",
                  buf, pc_class_types[(int)GET_CLASS(ch)], rank[(int)GET_RANK(ch)][(int)GET_SEX(ch)]);
  sprintf(buf, "%s      /cYSEX:/cW %-8s/cY   WEIGHT:/cW %-3d       /cYHEIGHT:/cW %-3d\"/c0\r\n", 
                buf, genders[(int)GET_SEX(ch)], GET_WEIGHT(ch), GET_HEIGHT(ch));

  sprintf(buf, "%s    /cYITEMS:/cW [%-3d//%-3d]  /cYWEIGHT:/cW [%-3d//%-3d]/c0\r\n", buf, IS_CARRYING_N(ch), CAN_CARRY_N(ch), IS_CARRYING_W(ch), CAN_CARRY_W(ch));
  sprintf(buf, "%s    /cYCOINS:/c0 [%9d]P [%9d]G [%9d]S [%9d]C\r\n     /cYBANK:/c0 [%d] (Total: %d)\r\n", 
    buf, GET_PLATINUM(ch), GET_GOLD(ch), GET_SILVER(ch), GET_COPPER(ch),
    GET_BANK_GOLD(ch), convert_all_to_copper(ch) + GET_BANK_GOLD(ch));

  if(!IS_JOBLESS(ch)) {
    sprintf(buf, "%s   /cYPOINTS:/c0 [%d]\r\n", buf, GET_POINTS(ch));
  }


  if(IS_GOBLIN_SLAYER(ch)) {
    sprintf(buf, "%sYou have killed %d goblins.\r\n", buf, GET_GOBLINS_SLAYED(ch));
  }
  if(IS_PIG_FARMER(ch)) {
    sprintf(buf, "%sYou have raised %d pigs, totaling %d pounds.\r\n", buf, GET_PIGS_RAISED(ch), GET_POUNDS_OF_PIG(ch));
    sprintf(buf, "%sThe heaviest pig you raised weighed %d pounds.\r\n", buf, GET_PIG_WEIGHT(ch));
 
  }
  if(IS_FISHERMAN(ch)) {
    for(i = 0;i < MAX_FISH;i++) {
      if(ch->player_specials->saved.types_of_fish[i][0]) { 
        fish++; 
        caught += ch->player_specials->saved.types_of_fish[i][0];
      }
    }
    sprintf(buf, "%sYou have caught %d fish, comprising %d of %d species.\r\n", buf, caught, fish, MAX_FISH);

  }


  if(IS_SET(PLR_FLAGS(ch), PLR_PIG_DONE))
    sprintf(buf, "%sYou have completed your job as a pig farmer.\r\n", buf);
  if(IS_SET(PLR_FLAGS(ch), PLR_GOB_DONE))
    sprintf(buf, "%sYou have completed your job as a goblin slayer.\r\n", buf);
  if(IS_SET(PLR_FLAGS(ch), PLR_FISH_DONE))
    sprintf(buf, "%sYou have completed your job as a fisherman.\r\n", buf);
...
*/
  ...
  if (PLR_FLAGGED(ch, PLR_BOT))
    strcat(buf, "You are a bot.\r\n");
  send_to_char(buf, ch);
}
  ...
  if (OUTSIDE(ch)) {
    sprintf(buf, "The sky is %s and %s.\r\n", sky_look[weather_info.sky],
	    (weather_info.change >= 0 ? "you feel a warm wind from south" :
	     "you have a feeling bad weather is due"));
    send_to_char(buf, ch);
  } else
...
ACMD(do_who)
{
  ...
  for (d = descriptor_list; d; d = d->next) {
    ...
    if (d->original)
      ...
      sprintf(buf, "%s %s",
	      GET_NAME(tch),
	      no_house_noble_houses[(int)GET_HOUSE(tch)]);
      }
      else {
         sprintf(buf, "%s %s %s", rank[(int)GET_RANK(tch)][(int)GET_SEX(tch)],
              GET_NAME(tch),
              no_house_noble_houses[(int)GET_HOUSE(tch)]); 
      }
  ...
  if (!*arg) {
	...
	if (i && CAN_SEE(ch, i) && (i->in_room != NOWHERE)) {
	  if (d->original)
	    ...
	    sprintf(buf, "%-20s - [%5d] %s (%s)\r\n", GET_NAME(i),
		    GET_ROOM_VNUM(IN_ROOM(i)), world[i->in_room].name, zone_table[world[i->in_room].zone].name);
	  send_to_char(buf, ch);
	}
      }
  } else {
	...
	found = 1;
	sprintf(buf, "M%3d. %-25s - [%5d] %s (%s)\r\n", ++num, GET_NAME(i),
		GET_ROOM_VNUM(IN_ROOM(i)), world[IN_ROOM(i)].name, zone_table[world[i->in_room].zone].name);
	send_to_char(buf, ch);
      }
...
ACMD(do_toggle)
{
  ...
  sprintf(buf,
	  ...
	  ONOFF(!PRF_FLAGGED(ch, PRF_NOGRATZ)),
          ...
          ONOFF(PRF_FLAGGED(ch, PRF_AUTOBAIT)),
	  ctypes[COLOR_LEV(ch)]);
...
ACMD(do_scan)
{
  ...
  if (found == 0) {
    send_to_char("Nobody anywhere near you.\r\n", ch);
  }
  ch->in_room = is_in;
  if(found) {
    improve_skill(ch, SKILL_SCAN, found);
  }
}
...
ACMD(do_skills) {
 ...
 if(GET_LEVEL(ch) >= LVL_GOD) {
  victim = get_char_vis(ch, arg, FIND_CHAR_WORLD);
  if(!victim) {  
    send_to_char("No such person around.\r\n", ch);
    return;
  }

 strcpy(buf, "They know of the following Skills:\r\n");
 strcpy(buf2, buf);

 for (i = MAX_SPELLS +1; i < MAX_SKILLS+1; i++)
 {
   if(GET_SKILL(victim, i)) {
      sprintf(buf, "%-20s (%d)\r\n", skill_names[i - MAX_SPELLS], GET_SKILL(victim, i));
 ...
 for (i = MAX_SPELLS +1; i < MAX_SKILLS+1; i++)
 {
   if (GET_SKILL(ch, i) > 0)
   {
//      sprintf(buf, "%-20s (%d)\r\n", skill_names[i - MAX_SPELLS], GET_SKILL(ch, i));
    sprintf(buf, "%-20s %s\r\n", skill_names[i - MAX_SPELLS], howgood(GET_SKILL(ch, i)));
    strcat(buf2, buf);
   }
 }
 page_string(ch->desc, buf2, 1);
...
ACMD(do_compare)
{
    ...
    else
    {
        ...
        case ITEM_POLE:
            value1 = GET_OBJ_WEIGHT(obj1);
            value2 = GET_OBJ_WEIGHT(obj2);
            break;
...
ACMD(do_fish) { 
   int i = 0, fish = 0, caught = 0;

    if(GET_CURR_TIP(ch) == 10) {
       GET_CURR_TIP(ch)++;
    }

    sprintf(buf, "You have caught the following fish:\r\n\r\n");
    for(i = 0;i < MAX_FISH;i++) {
      if(ch->player_specials->saved.types_of_fish[i][0]) { 
         sprintf(buf, "%s%-30s: [%-3d|%3d Pound Personal Record]\r\n",  buf, fish_names[i], ch->player_specials->saved.types_of_fish[i][0], ch->player_specials->saved.types_of_fish[i][1]); 

         fish++; 
         caught += ch->player_specials->saved.types_of_fish[i][0];
      }
    }
    sprintf(buf, "%s\r\nYou have caught %d fish, comprising %d of %d species.\r\n", buf, caught, fish, MAX_FISH);
    page_string(ch->desc, buf, TRUE);
}
...
void make_who2html(void) {
   ...
   if ((opf = fopen("//home//aarait//public_html//who.txt", "w")) == 0) {
     log("Problem with who_2html()");
  ...
  for (d = descriptor_list; d; d = d->next) {
   ...
   i++;
   sprintf(buf, "%s %s\n", 
   GET_NAME(tch), no_house_noble_houses[(int)GET_HOUSE(tch)]);
   fprintf(opf, buf);
   }
  }
  if(!i) {
    fprintf(opf, "No One is Playing!\n");
  }
   fclose(opf);
}
