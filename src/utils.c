...
void stop_follower(struct char_data * ch)
{
  ...
  if (ch->master == NULL) {
    sprintf(buf, "stop_follower: %s", GET_NAME(ch));
    log(buf);
    core_dump();
    return;
  }
...
void improve_stat(struct char_data *ch, int stat)
{
  ...
  if (number(1, 99999) < GET_WIS(ch) + GET_INT(ch) && !number(0, 100))
     return;
  ...
  default:
  break;
  sprintf(statbuf, "You feel your %s improving.", stat_names[stat]);
  send_to_char(statbuf, ch);
...
void improve_skill(struct char_data *ch, int skill, int bonus)
{
  ...
  if(IS_NPC(ch) || IS_MOB(ch)) { return; }
 
  percent = GET_SKILL(ch, skill);

  if(GET_SKILL(ch, skill) > 99) { return; }

  if (number(1, 250) > GET_WIS(ch) + GET_INT(ch))
     return;
  newpercent = number(1, 3 + bonus);
  percent += newpercent;
 
  SET_SKILL(ch, skill, percent);
  if (newpercent >= 4) {
     send_to_char("You feel very skillful today.\r\n", ch);
  }
  else {
    send_to_char("You feel skillful today.\r\n", ch);
  }
  if(PRF_FLAGGED(ch, PRF_DEBUG)) {
    sprintf(buf, "%s is now %d\r\n", skill_names[skill - MAX_SPELLS], GET_SKILL(ch, skill));
    send_to_char(buf, ch);
  }
}
...
void improve_hp(struct char_data *ch)
{
  ...
  if(GET_MAX_HIT(ch) > number(90, 110)) {
    return;
  }

  if (number(1, 450) > (GET_WIS(ch) + GET_INT(ch)))
     return;
  newpercent = number(1, 3);
  if (newpercent == 3) {
     ch->points.max_hit += number(1, GET_CON(ch) / 2);
     send_to_char("You feel very youthful today.\r\n", ch);
  }
}
...
void improve_points(struct char_data *ch, int bonus)
{
  ...
  GET_POINTS(ch) += newpercent;

  send_to_char("You earn some points.\r\n", ch);
