...
void auto_equip(struct char_data *ch, struct obj_data *obj, int location)
{
  ...
  if (location > 0) {	/* Was wearing it. */
	...
	break;
      if (CAN_WEAR(obj, ITEM_WEAR_WIELD) && GET_OBJ_TYPE(obj) == ITEM_WEAPON)
	break;
      location = LOC_INVENTORY;
      break;
    default:
...
int Crash_load(struct char_data * ch)
{
  ...
  if (rent.rentcode == RENT_RENTED || rent.rentcode == RENT_TIMEDOUT) {
    ...
    if (cost > convert_all_to_copper(ch) + GET_BANK_GOLD(ch)) {
      fclose(fl);
      ...
      // BEEP BUG
//      GET_BANK_GOLD(ch) -= MAX(cost - convert_all_to_copper(ch), 0);
//      add_money_to_char(ch, convert_all_to_copper(ch) - cost, GOLD_COINS);
...
// BEEP
    while ((cost > convert_all_to_copper(ch) + GET_BANK_GOLD(ch)) && ch->carrying) {
  ...
  rent.time = time(0);
  rent.plat = GET_MONEY(ch, PLAT_COINS);
  rent.gold = GET_MONEY(ch, GOLD_COINS);
  rent.silver = GET_MONEY(ch, SILVER_COINS);
  rent.copper = GET_MONEY(ch, COPPER_COINS);
...
void Crash_cryosave(struct char_data * ch, int cost)
{
  ...
  if (cost > (convert_all_to_copper(ch) + GET_BANK_GOLD(ch))) {
    GET_BANK_GOLD(ch) = 0;
    for (j = 0; j < 4; j++)
      remove_money_from_char(ch, GET_MONEY(ch, j), j);
  } else if (cost > convert_all_to_copper(ch)) {
    cost -= convert_all_to_copper(ch);
    for (j = 0; j < 4; j++)
      remove_money_from_char(ch, GET_MONEY(ch, j), j);
    GET_BANK_GOLD(ch) -= cost;
  } else
    make_change(ch, cost, BUYING);
			      ...
			      long cost)
{
  ...
  rent_deadline = ((convert_all_to_copper(ch) + GET_BANK_GOLD(ch)) / cost);
		         ...
		         int display, int factor)
{
  ...
  if (display) {
    ...
    act(buf, FALSE, receptionist, 0, ch, TO_VICT);
 sprintf(buf, "$n tells you, 'For a total of %ld copper coins%s.'",
	    totalcost, (factor == RENT_FACTOR ? " per day" : ""));

    act(buf, FALSE, receptionist, 0, ch, TO_VICT);
     if (totalcost > convert_all_to_copper(ch) + GET_BANK_GOLD(ch)) {
		         ...
		         int cmd, char *arg, int mode)
{
  ...
  if (CMD_IS("rent")) {
    if (!(cost = Crash_offer_rent(ch, recep, FALSE, mode)))
      return (TRUE);
    if (mode == RENT_FACTOR)
sprintf(buf, "$n tells you, 'Rent will cost you %d copper coins per day.'", cost);

    else if (mode == CRYO_FACTOR)
sprintf(buf, "$n tells you, 'It will cost you %d copper coins to be frozen.'", cost);
...
cost, convert_all_to_copper(ch) + GET_BANK_GOLD(ch));
    ...
    if(GET_LEVEL(ch) < LVL_GOD) {
      ...
      sprintf(buf, "/cG%s has left the game./c0\r\n", GET_NAME(ch));
      send_to_all(buf);
    }
