...
void perform_drop_gold(struct char_data * ch, int amount, int cointype, byte mode, room_rnum RDR);
...
void get_check_money(struct char_data * ch, struct obj_data * obj)
{
  ...
  if (GET_OBJ_TYPE(obj) != ITEM_MONEY)
    ...
    if(GET_OBJ_VAL(obj, PLAT_COINS)) {
       add_money_to_char(ch, GET_OBJ_VAL(obj, PLAT_COINS), PLAT_COINS);
       sprintf(buf, "There were %d platinum coins.\r\n", GET_OBJ_VAL(obj, PLAT_COINS));
       send_to_char(buf, ch);
    }
    if(GET_OBJ_VAL(obj, GOLD_COINS)) {
       add_money_to_char(ch, GET_OBJ_VAL(obj, GOLD_COINS), GOLD_COINS);
       sprintf(buf, "There were %d gold coins.\r\n", GET_OBJ_VAL(obj, GOLD_COINS));
       send_to_char(buf, ch);
    }
    if(GET_OBJ_VAL(obj, SILVER_COINS)) {
       add_money_to_char(ch, GET_OBJ_VAL(obj, SILVER_COINS), SILVER_COINS);
       sprintf(buf, "There were %d silver coins.\r\n", GET_OBJ_VAL(obj, SILVER_COINS));
       send_to_char(buf, ch);
    }
    if(GET_OBJ_VAL(obj, COPPER_COINS)) {
       add_money_to_char(ch, GET_OBJ_VAL(obj, COPPER_COINS), COPPER_COINS);
       sprintf(buf, "There were %d copper coins.\r\n", GET_OBJ_VAL(obj, COPPER_COINS));
       send_to_char(buf, ch);
    }
...
void perform_drop_gold(struct char_data * ch, int amount, int cointype,
		            byte mode, room_rnum RDR)
{
  ...
  if (cointype < 4)
	  money[cointype] = amount;
  else {
   log("SYSERR: Mixed coin type passed to perform_drop_gold() - unimp'd");
   send_to_char("Eh? What kind of coins?", ch);
   return;
  };

  if (amount <= 0)
    send_to_char("Heh heh heh.. we are jolly funny today, eh?\r\n", ch);
  else if ( GET_MONEY(ch, cointype) < amount ){
    sprintf(buf, "You don't have that many %s coins!\r\n", coin_types[cointype]);
    send_to_char(buf, ch);
  } else {
    if (mode != SCMD_JUNK) {
      WAIT_STATE(ch, PULSE_VIOLENCE);	/* to prevent coin-bombing */
      obj = create_money(money[PLAT_COINS],money[GOLD_COINS],money[SILVER_COINS],money[COPPER_COINS], cointype);
      if (mode == SCMD_DONATE) {
				sprintf(buf, "You throw some %s into the air where it disappears in a puff of smoke!\r\n", coin_types[cointype]);
				send_to_char(buf, ch);
				sprintf(buf, "$n throws some %s into the air where it disappears in a puff of smoke!", coin_types[cointype]);
				act(buf,FALSE, ch, 0, 0, TO_ROOM);
				obj_to_room(obj, RDR);
				act("$p suddenly appears in a puff of orange smoke!", 0, 0, obj, 0, TO_ROOM);
		} else {
			sprintf(buf, "You drop some %s coins.\r\n", coin_types[cointype]);
 	 		send_to_char(buf, ch);
			sprintf(buf, "$n drops %s.", money_desc(amount, cointype));
			act(buf, TRUE, ch, 0, 0, TO_ROOM);
			obj_to_room(obj, ch->in_room);
      }
    } else {
      sprintf(buf, "$n drops %s which disappears in a puff of smoke!",
	      money_desc(amount, cointype));
      act(buf, FALSE, ch, 0, 0, TO_ROOM);
      send_to_char("You drop some gold which disappears in a puff of smoke!\r\n", ch);
    }
    remove_money_from_char(ch, amount, cointype);
  }
}
...
*/
  ...
  if (!*arg) {
    ...
    one_argument(argument, arg);
		if (!str_cmp("copper", arg) || is_abbrev(arg, "copper"))
      perform_drop_gold(ch, multi, COPPER_COINS, mode, RDR);
		else if (!str_cmp("silver", arg) || is_abbrev(arg, "silver"))
      perform_drop_gold(ch, multi, SILVER_COINS, mode, RDR);
		else if (!str_cmp("gold", arg) || is_abbrev(arg, "gold"))
      perform_drop_gold(ch, multi, GOLD_COINS, mode, RDR);
		else if (!str_cmp("platinum", arg) || is_abbrev(arg, "platinum"))
      perform_drop_gold(ch, multi, PLAT_COINS, mode, RDR);
		       ...
		       struct obj_data * obj)
{
  ...
  if (IS_CARRYING_N(vict) >= CAN_CARRY_N(vict) && !IS_NPC(ch)) {
		            ...
		            int amount, int type)
{
  ...
  if ((GET_MONEY(ch, type) < amount) && (IS_NPC(ch) || (GET_LEVEL(ch) < LVL_GOD))) {
    sprintf(buf,"You don't have that many %s coins!\r\n", coin_types[type]);
    send_to_char(buf, ch);
    return;
  }
  send_to_char(OK, ch);
  sprintf(buf, "$n gives you %d %s coin%s.", amount, coin_types[type], amount == 1 ? "" : "s");
  act(buf, FALSE, ch, 0, vict, TO_VICT);
  sprintf(buf, "$n gives %s to $N.", money_desc(amount, type));
  act(buf, TRUE, ch, 0, vict, TO_NOTVICT);

  remove_money_from_char(ch, amount, type);
  add_money_to_char(vict, amount, type);
...
ACMD(do_give)
{
  ...
  if (!*arg)
    ...
    if (!str_cmp("platinum", arg)  || (is_abbrev(arg, "platinum") == 1) || !str_cmp("silver", arg)
			|| (is_abbrev(arg, "silver") == 1) || !str_cmp("copper", arg) || (is_abbrev(arg, "copper") == 1)
			|| !str_cmp("gold", arg) || (is_abbrev(arg, "gold") == 1)) {
      one_argument(argument, arg1);
	      if ((vict = give_find_vict(ch, arg1)) != NULL){
          if (!str_cmp("silver", arg) || (is_abbrev(arg, "silver") == 1))
	  	    	perform_give_gold(ch, vict, amount, SILVER_COINS);
          else if (!str_cmp("platinum", arg) || (is_abbrev(arg, "platinum") == 1))
	  	    	perform_give_gold(ch, vict, amount, PLAT_COINS);
          else if (!str_cmp("gold", arg) || (is_abbrev(arg, "gold") == 1))
	  	    	perform_give_gold(ch, vict, amount, GOLD_COINS);
          else if (!str_cmp("copper", arg) || (is_abbrev(arg, "copper") == 1))
	  	    	perform_give_gold(ch, vict, amount, COPPER_COINS);
        }
...
void perform_wear(struct char_data * ch, struct obj_data * obj, int where)
{
  ...
  const char *already_wearing[] = {
    ...
    "You're are already wielding offhand.\r\n"
  };
...
void perform_remove(struct char_data * ch, int pos)
{
  ...
  if (!(obj = GET_EQ(ch, pos)))
    ...
    if(pos == WEAR_WIELD && AFF_FLAGGED(ch, AFF_FISHING)) {
    	act("You stop fishing.", FALSE, ch, obj, 0, TO_CHAR);
    	act("$n stops fishing.", TRUE, ch, obj, 0, TO_ROOM);
...
ACMD(do_cast)
{
  ...
  if (AFF_FLAGGED(ch, AFF_FISHING) || GET_REELIN(ch)) {
    send_to_char("You are already fishing!\r\n", ch);
    return;
  }
  if (!(pole = GET_EQ(ch, WEAR_WIELD)) ||
      (GET_OBJ_TYPE(pole) != ITEM_POLE)) {
    send_to_char("You need to be wielding a fishing pole first.\r\n", ch);
    return;
  }

  // Don't ask what I was thinking here.
  if (world[ch->in_room].sector_type != SECT_WATER_NOSWIM) { 
      if (world[ch->in_room].sector_type != SECT_WATER_SWIM) {
      if (world[ch->in_room].sector_type != SECT_BRIDGE) {
        send_to_char("You look around for a place to cast, but there is no water here.\r\n", ch);
        return;
      }
   }
  }
  ...
  if (fail <= 3) {
    ...
    send_to_char("You pull your arm back and try to cast out your line, but it\r\ngets all tangled up. Try again.\r\n", ch);
    act("$n pulls $s arm back, trying to cast $s fishing line out into the\r\nwater, but ends up just a bit tangled.\r\n",
         FALSE, ch, 0, 0, TO_ROOM);
    damage_objs_exact(ch, WEAR_WIELD);
    return;
  }

    if(GET_CURR_TIP(ch) == 3) {
       GET_CURR_TIP(ch)++;
    }

  send_to_char("You cast your line out into the water, hoping for a bite.\r\n",
ch);
  act("$n casts $s line out into the water, hoping to catch a fish.\r\n",
       FALSE, ch, 0, 0, TO_ROOM);
  GET_REELIN(ch) += 6;
  damage_objs_exact(ch, WEAR_WIELD);
  if(GET_EQ(ch, WEAR_WIELD)) {
   SET_BIT(AFF_FLAGS(ch), AFF_FISHING);
  }
  return;
}
...
ACMD(do_bait)
{
    ...
    if (arg1[0] == '\0' )
    {
        send_to_char("Bait what with what?\n\r", ch);
        return;
    }
    ...
    if (!(pole = get_obj_in_list_vis(ch, arg1, ch->carrying)))
    {
        send_to_char("You do not have that in your inventory.\n\r", ch);
        return;
    }

    if(GET_OBJ_TYPE(pole) != ITEM_POLE) { 
	send_to_char("That is not a pole.\r\n", ch);
	return;
    }

    if (!(bait = get_obj_in_list_vis(ch, arg2, ch->carrying)))
    {
        send_to_char("You do not have that kind of bait.\n\r", ch);
        return;
    }

    if((GET_OBJ_TYPE(bait) != ITEM_BAIT) && (GET_OBJ_TYPE(bait) != ITEM_FISH)) {
	send_to_char("You can not use that as bait.\r\n", ch);
	return;
    }

    if(GET_OBJ_COST(bait) > 7 && GET_OBJ_TYPE(bait) == ITEM_FISH) {
        send_to_char("You can not use that as bait.\r\n", ch);
        return;
    }

    if(GET_OBJ_VAL(pole, 0)) {
      send_to_char("It is already baited.\r\n", ch);
      return;
    }
    // It checks out, let us apply the bait to the hook.

    if(GET_CURR_TIP(ch) == 1) {
       GET_CURR_TIP(ch)++;
    }

    GET_OBJ_VAL(pole, 0) = GET_OBJ_VNUM(bait);
    GET_OBJ_VAL(pole, 1) = GET_OBJ_VAL(bait, 0);
    GET_OBJ_VAL(pole, 2) = GET_OBJ_COST(bait);
    sprintf(buf, "You bait %s with %s.\r\n", pole->short_description, bait->short_description);
    send_to_char(buf, ch);
    sprintf(buf, "%s baits %s with %s.\r\n", GET_NAME(ch), pole->short_description, bait->short_description);
    act(buf, FALSE, ch, 0, 0, TO_ROOM);
    obj_from_char(bait);
    extract_obj(bait);
...
ACMD(do_reelin)
{
   struct obj_data *fish_obj;
   struct char_data *fish = get_char_num(real_mobile(GET_FISHON(ch)));
   int minus = 2, record = 0, bonus = 0;
   char arg1[MAX_STRING_LENGTH];

   if(!GET_REELIN(ch) && !GET_FISHON(ch)) {
     send_to_char("But, you are not even fishing!\r\n", ch);
     return; 
   }
   ...
   if(!GET_FISHON(ch)) { 
       REMOVE_BIT(AFF_FLAGS(ch), AFF_FISHING);
       GET_REELIN(ch) = 0;
       send_to_char("You stop fishing and reel in your line.\r\n", ch);
       act("$n reels in $s line and stops fishing.", FALSE, ch, 0, 0, TO_ROOM);
       return;
   }
   ...
   if (is_abbrev(arg1, "normal")) { 
       send_to_char("You begin reeling in your line.\r\n", ch); 
       act("$n begins reeling in $s line.", FALSE, ch, 0, 0, TO_ROOM);
       minus = 3; 
   }
   else if (is_abbrev(arg1, "fast")) { 
       send_to_char("You begin reeling in your line very fast.\r\n", ch);
       act("$n begins reeling in $s line very fast.", FALSE, ch, 0, 0, TO_ROOM); 
       minus = 5; 
   }
   else {
       send_to_char("You start reeling in your line very slow.\r\n", ch);
       act("$n begins reeling in $s line very slow.", FALSE, ch, 0, 0, TO_ROOM);
       minus = 2;
   }


/*    if(GET_CURR_TIP(ch) == (5 || 6 || 7)) {
       GET_CURR_TIP(ch)++;
    }
*/
   if(GET_WEIGHT(fish) > number(GET_STR(ch) / 2, GET_STR(ch))) { 
      if(!number(0, 10)) { 
         sprintf(buf, "%s drags your line out further into the water.\r\n", GET_NAME(fish));
         send_to_char(buf, ch);
         sprintf(buf, "%s drags %s's line out further into the water.\r\n", GET_NAME(fish), GET_NAME(ch));         
         act(buf, FALSE, ch, 0, 0, TO_ROOM);
         GET_REELIN(ch) += 4;
      } 
   }
   
   if((GET_MOB_WEIGHT(fish) > (GET_STR(ch) * 2) + GET_ADD(ch)) && (!number(0, 20))) {
      GET_REELIN(ch) = 0;
      GET_FISHON(ch) = 0;
      sprintf(buf, "%s breaks your line.", GET_NAME(fish));
      act(buf, FALSE, ch, 0, fish, TO_CHAR);
      sprintf(buf, "%s breaks %s's line.", GET_NAME(fish), GET_NAME(ch));
      act(buf, FALSE, ch, 0, fish, TO_ROOM);
      damage_objs_exact(ch, WEAR_WIELD);
      return;
   }
   ...
   // They Caught it
      ...
      fish_obj = read_object(GET_FISHON(ch), VIRTUAL); 

      if(!fish_obj) {
        send_to_char("Bug, please report\r\n", ch);
        log("No fish_obj in do_reelin");
        return;
      }
      ...
      if(GET_MOB_WEIGHT(fish)) {
        ...
        GET_OBJ_VAL(fish_obj, 0) = number(15, 18); // BEEP 
      ...
      if(GET_MOB_WEIGHT(fish)) {
        sprintf(buf, "You catch %s that weighs about %d pounds!\r\n", GET_NAME(fish), GET_MOB_WEIGHT(fish));
        send_to_char(buf, ch); 
        sprintf(buf, "%s catches %s that weighs about %d pounds!\r\n", GET_NAME(ch), GET_NAME(fish), GET_MOB_WEIGHT(fish));
        act(buf, FALSE, ch, 0, 0, TO_ROOM);
      }
      else {
        sprintf(buf, "You catch %s that weighs less than one pound!\r\n", GET_NAME(fish));
        send_to_char(buf, ch);
        sprintf(buf, "%s catches %s that weighs less than one pound!\r\n", GET_NAME(ch), GET_NAME(fish));
        act(buf, FALSE, ch, 0, 0, TO_ROOM);
      }
...
//send_to_char(buf, ch);
      ...
      for(bonus = 0;bonus < MAX_FISH;bonus++) {

//sprintf(buf, "%d ", fish_vnums[bonus]);
//send_to_char(buf, ch);

        if(fish_vnums[bonus] == GET_MOB_VNUM(fish)) {
            ch->player_specials->saved.types_of_fish[bonus][0] += 1;
           if(GET_MOB_WEIGHT(fish) && GET_MOB_WEIGHT(fish) > ch->player_specials->saved.types_of_fish[bonus][1]) {
             ch->player_specials->saved.types_of_fish[bonus][1] = GET_MOB_WEIGHT(fish);
             record = 1;
           }
           break;
        }
      ...
      if(record) {
        send_to_char("/cYYou just broke your personal record!/c0\r\n", ch);
      }

      if(IS_FISHERMAN(ch)) {
        if(GET_MOB_WEIGHT(fish) > 10) { 
          improve_skill(ch, SKILL_FISHING, (2 + record));
          improve_points(ch, (2 + record));
        }
      }       


      for (fish = world[ch->in_room].people; fish; fish = fish->next_in_room) {
        if(GET_MOB_VNUM(fish) == GET_FISHON(ch)) {
            extract_char(fish);
            break;
        }
      }
      GET_REELIN(ch) = 0;
      GET_FISHON(ch) = 0;
      if(PRF_FLAGGED(ch, PRF_AUTOBAIT)) {
        command_interpreter(ch, "remove pole");
        command_interpreter(ch, "bait pole bait");
        command_interpreter(ch, "wield pole");
      }

      autoquest_trigger_check(ch, NULL, fish_obj, AQ_FISH);

   }
   else { 
      sprintf(buf, "You fight with %s as you reel it in closer.", GET_NAME(fish));
      act(buf, FALSE, ch, 0, fish, TO_CHAR);
      sprintf(buf, "%s fights with %s as they reel it in closer.", GET_NAME(ch), GET_NAME(fish)); 
      act(buf, FALSE, ch, 0, fish, TO_ROOM);
   }
   damage_objs_exact(ch, WEAR_WIELD);
...
ACMD(do_repair)
{
 ...
 if (GET_OBJ_CSLOTS(repair) == GET_OBJ_TSLOTS(repair)) {
  act("$n says, '$p seems to be in perfect condition!'", FALSE, ch, repair, 0,
  TO_ROOM); return;
 }

 improve_skill(ch, SKILL_REPAIR, 0);

 if (GET_OBJ_CSLOTS(repair) < 0 && GET_SKILL(ch, SKILL_REPAIR) < number(0, 101)) {
  act("$n tries to repair $p, but it crumbles
...
void damage_objs(struct char_data *ch) {
   ...
   for (j = 0; j < NUM_WEARS; j++) {
    ...
    condition = GET_OBJ_TSLOTS(ch->equipment[j]) * 2;

    if (number(0, condition) == 1) {
     if (ch->equipment[j]->obj_flags.total_slots != 0) {
      ch->equipment[j]->obj_flags.curr_slots--;
      sprintf(buf, "%s just got DAMAGED!\r\n", ch->equipment[j]->short_description);
      act(buf, FALSE, ch, 0, 0, TO_CHAR);
     }
    }

    if ((ch->equipment[j]->obj_flags.curr_slots == 0) && (ch->equipment[j]->obj_flags.total_slots != 0)) {
     sprintf(buf, "%s crumbles to dust as it wears out!\r\n", ch->equipment[j]->short_description);
     act(buf, FALSE, ch, 0, 0, TO_CHAR);

     if(GET_OBJ_TYPE(ch->equipment[j]) == ITEM_POLE) {
...
void damage_objs_exact(struct char_data *ch, int j) {
   int condition = 0;
    ...
    condition = GET_OBJ_TSLOTS(ch->equipment[j]);
...
ACMD(do_chum) {
  ...
  obj = get_obj_in_list_vis(ch, "chum", ch->carrying);

  if (!obj) {
      send_to_char("You do not have any chum to chum with!\r\n", ch); 
      return;
  }
  if(SECT(ch->in_room) != SECT_WATER_NOSWIM && SECT(ch->in_room) != SECT_WATER_SWIM) {
      send_to_char("Why would want to chum the ground? Are you fishing for field fish?\r\n", ch);
      return;
  }
  if(!GET_OBJ_VAL(obj, 0)) {
     send_to_char("The chum is all used up!\r\n", ch);
     return;
  } 
  
  act("You sprinkle some chum in the water.", FALSE, ch, 0, 0, TO_CHAR);
  act("$n sprinkles some chum in the water.", FALSE, ch, 0, 0, TO_ROOM);
                 
  GET_OBJ_VAL(obj, 0)--; 
  increase_chum(ch->in_room, 2);
}
...
ACMD(do_unbait)
{
    ...
    if(!GET_OBJ_VAL(pole, 0)) {
      send_to_char("It's not baited.\r\n", ch);
      return;
    }
    // It checks out, let us apply the bait to the hook.

    bait = read_object(GET_OBJ_VAL(pole, 0), VIRTUAL);

    GET_OBJ_VAL(pole, 0) = 0;
    GET_OBJ_VAL(pole, 1) = 0;
    GET_OBJ_VAL(pole, 2) = 0;
    sprintf(buf, "You remove %s from %s.\r\n", bait->short_description, pole->short_description);
    send_to_char(buf, ch);
    sprintf(buf, "%s removes %s from %s.\r\n", GET_NAME(ch), bait->short_description, pole->short_description);
    act(buf, FALSE, ch, 0, 0, TO_ROOM);
    obj_to_char(bait, ch);
