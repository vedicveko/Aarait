...
extern struct index_data *obj_index, *mob_index;
...
ACMD(do_recall)
{
  int silver_take = (GET_POINTS(ch) * 1);
  ...
  if(AFF_FLAGGED(ch, AFF_FISHING) || GET_FISHON(ch)) {
    ...
    char_from_room(ch);
    char_to_room(ch, real_room(104));
    act("$n appears suddenly.", TRUE, ch, 0, 0, TO_ROOM);
    look_at_room(ch, 0);
    return;
  }
  else {
    sprintf(buf, "Recalling for %d coins...\r\n", silver_take);
    send_to_char(buf, ch);
    if (convert_all_to_copper(ch) < silver_take)
{
    ...
    make_change(ch, silver_take, BUYING);
  if(AFF_FLAGGED(ch, AFF_FISHING) || GET_FISHON(ch)) {
    REMOVE_BIT(AFF_FLAGS(ch), AFF_FISHING);
    GET_FISHON(ch) = 0;
    GET_REELIN(ch) = 0;
