...
int is_ok_char(struct char_data * keeper, struct char_data * ch, int shop_nr)
{
  ...
  if ((IS_PIG_FARMER(ch) && NOTRADE_PIG_FARMER(shop_nr)) ||
     (IS_FISHERMAN(ch) && NOTRADE_FISHERMAN(shop_nr)) ||
     (IS_GOBLIN_SLAYER(ch) && NOTRADE_GOBLIN_SLAYER(shop_nr))) {
		       ...
		       struct char_data * keeper, int shop_nr)
{
  ...
  if ((buy_price(obj, shop_nr) > convert_all_to_copper(ch)) && !IS_GOD(ch)) {
  ...
  while ((obj) && 
         (convert_all_to_copper(ch) >= (buy_price(obj, shop_nr) + goldamt))
    ...
    if (!IS_GOD(ch))
         make_change(keeper, goldamt, BUYING);
  ...
  if (bought < buynum) {
    ...
    else if (convert_all_to_copper(ch) < goldamt)
  ...
  if (!IS_GOD(ch))
    make_change(ch, goldamt, BUYING);

  sprintf(tempstr, times_message(ch->carrying, 0, bought));
  sprintf(buf, "$n buys something.");
  ...
  if (SHOP_USES_BANK(shop_nr))
    if (convert_all_to_copper(keeper) > MAX_OUTSIDE_BANK) {
      act("$n drops some coins in $s safe.", FALSE, keeper, 0, 0, TO_ROOM);
      SHOP_BANK(shop_nr) += (convert_all_to_copper(keeper) - MAX_OUTSIDE_BANK);
      remove_money_from_char(keeper, GET_MONEY(keeper, PLAT_COINS), PLAT_COINS);
      remove_money_from_char(keeper, GET_MONEY(keeper, GOLD_COINS), GOLD_COINS);
  remove_money_from_char(keeper, GET_MONEY(keeper, SILVER_COINS), SILVER_COINS);
  remove_money_from_char(keeper, GET_MONEY(keeper, COPPER_COINS), COPPER_COINS);
     add_money_to_char(keeper, (MAX_OUTSIDE_BANK/COPPER_PER_PLAT), PLAT_COINS);
        }
}
		        ...
		        struct char_data * keeper, int shop_nr)
{
  ...
  if (convert_all_to_copper(keeper) + SHOP_BANK(shop_nr) < sell_price(ch, obj, shop_nr)) {
  ...
  while ((obj) && (convert_all_to_copper(keeper) + SHOP_BANK(shop_nr) >=
		   sell_price(ch, obj, shop_nr)) && (sold < sellnum)) {

    sold++;

    goldamt += sell_price(ch, obj, shop_nr);
    make_change(keeper, sell_price(ch, obj, shop_nr), BUYING);


// Makes the obj avail to be sold.
...
//    obj = get_selling_obj(ch, name, keeper, shop_nr, FALSE);
  ...
  if (sold < sellnum) {
    ...
    else if (convert_all_to_copper(keeper) + SHOP_BANK(shop_nr) <
  ...
  make_change(ch, goldamt, SELLING);
  strcpy(tempstr, times_message(0, name, sold));
  act("$n sells something.", FALSE, ch, 0, 0, TO_ROOM);
  ...
  if (convert_all_to_copper(keeper) < MIN_OUTSIDE_BANK) {
    goldamt = MIN((MAX_OUTSIDE_BANK - convert_all_to_copper(keeper)), SHOP_BANK(shop_nr));
    SHOP_BANK(shop_nr) -= goldamt;
    add_money_to_char(keeper, goldamt, COPPER_COINS);
  }
		         ...
		         struct char_data * keeper, int shop_nr)
{
  ...
  list_from_copper(sell_price(ch, obj, shop_nr), cost, UNCONDENSED_LIST);

  sprintf(buf, "%s I'll give you %s for that!", GET_NAME(ch), cost);
  do_tell(keeper, buf, cmd_tell, 0);
  return;
}
...
char *list_object(struct obj_data * obj, int cnt, int index, int shop_nr)
{
  ...
  list_from_copper(buy_price(obj, shop_nr), buf4, CONDENSED_LIST);
...
SPECIAL(shop_keeper)
{
  ...
  if (!AWAKE(keeper))
    return (FALSE);
  if(AFF_FLAGGED(keeper, AFF_INVISIBLE))
    return (FALSE);
...
void list_detailed_shop(struct char_data * ch, int shop_nr)
{
  ...
  if (SHOP_KEEPER(shop_nr) >= 0) {
	...
	convert_all_to_copper(k), SHOP_BANK(shop_nr), convert_all_to_copper(k) + SHOP_BANK(shop_nr));
    }
  } else
    strcat(buf, "<NONE>\r\n");
  send_to_char(buf, ch);
