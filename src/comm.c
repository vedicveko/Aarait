...
extern char *print_rp_hit(struct char_data *ch);
...
// GET_JOB_TIME is increased by wolf_hunger()
//      wolf_hunger();
  }
  if (!(pulse % (2 * 60 * PASSES_PER_SEC))) {	/* 5 minutes */
   // record_usage();
//    zone_emotes();
    update_chum();
    make_who2html();
...
char *make_prompt(struct descriptor_data *d)
{
  ...
  if (d->showstr_count) {
    sprintf(prompt,
	    "\r/cW[ Return to continue, (q)uit, (r)efresh, (b)ack, or page number (%d|%d) ]",
    ...
    *prompt = '\0';
    ...
    if(GET_CLASS(d->character) != CLASS_JOBLESS) {
      count += sprintf(prompt + count, "/cWP: %d/c0 ", GET_POINTS(d->character));
    }

    count += sprintf(prompt + count, "/cW>/c0 ");
...
int process_output(struct descriptor_data *t)
{
  ...
  i[MAX_SOCK_BUF-1]=(char)0;
  proc_color(i, clvl); /* from color.c */
		...
		const void *vict_obj, const struct char_data *to)
{
  ...
  for (;;) {
	...
	break;
      default:
	log("SYSERR: Illegal $-code to act(): %c, %s", *orig, GET_NAME(ch));
	log("SYSERR: %s", orig);
        i = "";
	break;
      }
