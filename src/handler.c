...
void obj_from_char(struct obj_data * object)
{
  ...
  if (object == NULL) {
   ...
   if ((object) == (object->carried_by->carrying)) {                
      ...
      while (temp && (temp->next_content != (object))) {
...
void update_char_objects(struct char_data * ch)
{
  ...
  if (GET_EQ(ch, WEAR_LIGHT) != NULL)
    if (GET_OBJ_TYPE(GET_EQ(ch, WEAR_LIGHT)) == ITEM_LIGHT)
      if (GET_OBJ_VAL(GET_EQ(ch, WEAR_LIGHT), 2) > 0 && IS_DARK(ch->in_room)) {
...
struct obj_data *create_money(int plat, int gold, int silver, int copper, int type)
{
  struct obj_data *obj;
  ...
  GET_OBJ_COST(obj) = ((plat * COPPER_PER_PLAT) + (gold * COPPER_PER_GOLD) + (silver * COPPER_PER_SILVER) + copper);
