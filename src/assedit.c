 ...
 *buf2 = '\0';
 ...
 for (d = descriptor_list; d; d = d->next) {
     ...
     send_to_char("Assemblies are already being editted by someone.\r\n", ch);
     return;
     }
   }
...
void assedit_disp_menu(struct descriptor_data *d)
{
 ...
 sprinttype( OLC_ASSEDIT(d)->uchAssemblyType, AssemblyTypes, szAssmType );
...
{
 int pos = 0, YES = 1, NO = 0, i = 0,  counter, columns = 0;
...
send_to_char(buf, d->character);
      ...
      for (counter = 0; counter < MAX_ASSM; counter++) {
           ...
           send_to_char("Enter the assembly type : ", d->character);
...
/*        free(OLC_ASSEDIT(d)->pComponents); */
 ...
 mudlog("SYSERR: OLC assedit_parse(): Reached default case!", BRF, LVL_GOD, TRUE);
 send_to_char("Opps...\r\n", d->character);
 STATE(d) = CON_PLAYING;
 break;
 }
...
void nodigit(struct descriptor_data *d)
{
  send_to_char("Usage: assedit <vnum>\r\n", d->character);
  send_to_char("     : assedit new <vnum>\r\n", d->character);
  send_to_char("     : assedit delete <vnum>\r\n", d->character);
  return;
}
